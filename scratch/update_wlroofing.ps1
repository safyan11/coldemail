$file = "c:\Users\safim\OneDrive\Desktop\coldemail\coldemail\WLRoofingLimited\index.html"
$content = Get-Content $file -Raw

# Batch 1: Header to Hero
$content = $content -replace '<title>Business template</title>', '<title>WL Roofing Limited</title>'
$content = $content -replace '<p class="_14px-text">\+923347200660</p>', '<p class="_14px-text">085 103 4237</p>'
$content = $content -replace '<p class="_14px-text break">info@theriss\.net</p>', '<p class="_14px-text break">wlroofingltd@gmail.com</p>'
$content = $content -replace '<p class="_15px-400">info@theriss\.net</p>', '<p class="_15px-400">wlroofingltd@gmail.com</p>'
$content = $content -replace '<p class="_15px-400">\+923347200660</p>', '<p class="_15px-400">085 103 4237</p>'
$content = $content -replace '<p class="_15px-400">9261 Candice Trail,  <br />North Carolina, United States</p>', '<p class="_15px-400">105 Brú Na Gruadán, Newcastle,<br />Limerick, V94 TX46</p>'
$content = $content -replace '<h1 class="hero-title">Expert Plumbing Solutions for Every Home</h1>', '<h1 class="hero-title">Trusted Irish Roofing Solutions in Limerick</h1>'
$content = $content -replace 'Lorem ipsum dolor sit amet, consectetur adipiscing elit\. Suspendisse\s*varius enim in eros elementum tristique\. Duis cursus, mi quis viverra ornare\.', 'WL Roofing Limited is a 100% Irish, family-owned roofing company providing expert repairs and installations throughout Limerick.'
$content = $content -replace '<p class="_14px-text">Mon to Fri: 08:00 - 19:00</p>', '<p class="_14px-text">Mon to Thu: 08:00 - 18:00</p>'

# Batch 2: Why Choose Us
$content = $content -replace 'With years of experience, our certified plumbers deliver top-quality service\s*on every job, big or small\.', 'As a family-owned Irish company, we take pride in delivering top-quality roofing service on every job.'
$content = $content -replace '>24/7 Availability<', '>Local Irish Experts<'
$content = $content -replace 'Plumbing issues can happen anytime\. That&\#x27;s why\s*we&\#x27;re available around the clock to handle your emergencies\.', 'Based directly in Newcastle, Limerick, we respond quickly to protect your home from the Irish weather.'
$content = $content -replace 'No hidden fees—just honest, transparent pricing so you know exactly what\s*you&\#x27;re paying for before we start\.', 'We provide clear, honest estimates for all roofing projects, so you know exactly what to expect before work starts.'
...
$content = $content -replace '>Committed to Delivering Quality and Innovation<', '>100% Irish-Owned Family Roofing Experts<'
$content = $content -replace 'Lorem ipsum dolor sit amet, consectetur adipiscing elit\. Sed do\s*eiusmod tempor incididunt ut labore et dolore magna aliqua\. Ut enim ad minim veniam\.', 'Based in Limerick, WL Roofing Limited is dedicated to providing superior roofing solutions built to last in the Irish climate.'
$content = $content -replace 'ed do eiusmod tempor incididunt ut labore et dolore', 'Provide secure and durable roofing for homes across Limerick.'

# Batch 5: Stats Header & Numbers & 3 Images Block
$content = $content -replace '>Our Stats at a Glance<', '>Proven Roofing Experience<'
$content = $content -replace 'Lorem ipsum dolor sit amet, consectetur adipiscing elit\.\s*Suspendisse varius enim in eros elementum tristique\.', 'Our commitment to quality local service speaks for itself.'
...
$content = $content -replace '>Team Member<', '>Irish Owned<'
...
$content = $content -replace '>Ideal for minor repairs and basic installations\.<', '>Request an estimate for minor roof repairs.<'
...
$content = $content -replace 'We offer a full range of plumbing services including leak repair, drain cleaning, water heater installation, pipe replacement, and emergency services for both residential and commercial properties\.', 'We provide roofing and guttering services across Limerick.'
$content = $content -replace 'Do you provide emergency plumbing services\?', 'Are you a local business?'
$content = $content -replace 'Yes, we offer 24/7 emergency plumbing services\. Our technicians are ready to respond to urgent issues like burst pipes, severe leaks, and overflowing toilets at any time of day or night\.', 'Yes, WL Roofing Limited is a 100% Irish, family-owned business.'

# Batch 10: Footer Section
$content = $content -replace '>Copyright © 2024 Theriss\. All Rights Reserved\.<', '>Copyright © 2024 WL Roofing Limited. All Rights Reserved.<'
